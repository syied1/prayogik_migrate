 ...
 */
async function isCompositeSlugTaken(modelInstance, slugField, slugValue, compositeField, compositeValue) {
  const record = await modelInstance.findUnique({
    where: {
      [slugField]: slugValue,
 ...
 * @returns {string} - The unique slug
 */
export async function compositeSlugify(title, modelInstance, slugField, compositeField, compositeValue) {
  let slug = generateSlug(title); // Convert title to slug
  let slugExists = await isCompositeSlugTaken(modelInstance, slugField, slug, compositeField, compositeValue); // Check if slug exists
  let count = 1;

  // Loop to generate a unique slug by appending numbers if necessary
  while (slugExists) {
    const newSlug = `${slug}-${count}`;
    slugExists = await isCompositeSlugTaken(modelInstance, slugField, newSlug, compositeField, compositeValue);
    if (!slugExists) {
      slug = newSlug;
    }
    count++;
